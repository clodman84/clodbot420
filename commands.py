...
bot = commands.Bot(
    command_prefix="--", help_command=help_command, case_insensitive=True
)
live = asyncio.run(
    liveFormula.get_live("2021/2021-06-20_French_Grand_Prix/2021-06-20_Race/")
)
...
async def on_command_error(ctx, error):
    ...
    # For this error example we check to see where it came from...
        ...
        if ctx.command.qualified_name == "start":
            await ctx.send(
                "Please try again. Make sure you entered both *numbers* for break __and__ study"
            )
        else:
            pass
    elif isinstance(error, commands.MissingRequiredArgument):
        await ctx.author.send(
            f'Your command __**{ctx.command}**__ was incomplete, please use "--help {ctx.command}"'
        ...
        for i in exception:
            ...
            trace += "```"
        await ctx.send(
            "Error has been reported to <@793451663339290644> probably something stupid."
        )
...
async def mixtape(ctx, *args):
    ...
    embed = Embed(
        title="Enter Song Number",
        description=spotify.listSongs(search[1]),
        colour=0x1ED9C0,
    )
...
async def schedule(ctx, season="current"):
    ...
    if sched[0] == 200:
        data = {
            "No.": [a + 1 for a in range(len(sched[1]))],
            "Country": [],
            "Search ID": [],
            "Date": [],
        }
        for i in sched[1]:
            ...
            await ctx.send(f"```{utils.make_table(data)}```")
...
async def driverData(ctx, driver_id):
    ...
    embed.add_field(
        name="Nationality", value=result["driver"]["nationality"], inline=True
    )
...
async def wins(ctx, driver_id):
    ...
    try:
        ...
        table = (
            utils.make_table(data[0:middle]),
            utils.make_table(data[middle : 2 * middle]),
            utils.make_table(data[2 * middle : 3 * middle]),
            utils.make_table(data[3 * middle :]),
        )
    for i in table:
        await ctx.send(f"```\n{i}\n```")
...
async def lastSession(ctx):
    ...
    for index in range(len(timeData)):
        ...
        if i["LastLapTime"]["Value"] == "":
            ...
            embed = Embed(title=f"**{driver}**", colour=colour, description=description)
        ...
        try:
            if i["IntervalToPositionAhead"]["Catching"]:
                ...
                description += (
                    f"**{i['IntervalToPositionAhead']['Value']}** to driver ahead\n"
                )
        except KeyError:
        ...
        embed = Embed(
            title=f"**{driver}** {gap}", colour=colour, description=description
        )
        embed.set_image(url=image)
        await ctx.send(embed=embed)
...
async def nextRace(ctx):
    ...
    embed.add_field(
        name="Circuit",
        value=f"[{result['data']['Circuit']}]({result['data']['url']})",
        inline=False,
    )
...
async def feed(ctx, sat="3drimager"):
    ...
    if sat not in ["isrocast", "3dimager", "3drimager"]:
        await ctx.send(
            f"{sat} is not a valid satellite name. Use --satlist to check the list of valid satellite names"
        )
    ...
    for area in result:
        ...
        for aircraft in area[2]:
            if len(aircraft) == 17 and a < 11:
                try:
                    coords = (
                        str(round(aircraft[6], 1)) + " " + str(round(aircraft[5], 1))
                    )
                except TypeError:
                    coords = "Unknown"
                try:
        ...
        if a < 16:
            data.append(
                [
                    ...
                    datetime.fromtimestamp(plan["lastSeen"]).strftime(
                        "%H:%M:%S  %d/%m/%y"
                    ),
        ...
        if a < 21:
            data.append(
                [
                    ...
                    plan["icao24"],
                    plan["callsign"],
                    datetime.fromtimestamp(plan["firstSeen"]).strftime(
                        "%H:%M:%S  %d/%m/%y"
                    ),
                    plan["estDepartureAirport"],
                    datetime.fromtimestamp(plan["lastSeen"]).strftime(
