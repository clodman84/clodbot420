...
def generator(
        list_name,
):  # Generates curse words and spicy phrases to make the bot feel alive
    ...
    cursor.execute(
        f"select {list_name} from {list_name} where {list_name}_black = 'not used'"
    )
    ...
    while True:
        ...
        cursor.execute(
            f'update {list_name} set {list_name}_black = "used" where {list_name} = "{cancer}"'
        )
        mycon.commit()
        if list_name == "sites":
...
async def translate(txt, author):
    """Translates whatever text is passed through it into 5 different accents. Also strips the author names and
    returns only the name without the number """

    outputs = [
        [
            "Thee men talk too much i am did tire of translating thy words especially ",
            "Shakespeare_CUNT",
        ],
        ["Tired of translating I am, talk too much you do ", "Yoda_CUNT"],
        [
            "I am tired of translatin' you dudes. I just wanna smoke crack with ",
            "Valley_CUNT",
        ],
