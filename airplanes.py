...
async def iso(iso_code):
    ...
    url = 'https://opensky-network.org/api/states/all'
    box = [c.bbox for c in country_subunits_by_iso_code(iso_code)]  # the bounding box coords
    name = [c.name for c in country_subunits_by_iso_code(iso_code)]  # the name of the corresponding bounding box
    if box == [] or name == []:
        return None
    result = []
