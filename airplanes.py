...
async def iso(iso_code):
    ...
    url = "https://opensky-network.org/api/states/all"
    box = [
        c.bbox for c in country_subunits_by_iso_code(iso_code)
    ]  # the bounding box coords
    name = [
        c.name for c in country_subunits_by_iso_code(iso_code)
    ]  # the name of the corresponding bounding box
    if box == [] or name == []:
        return None
    result = []
...
async def history(icao):
    ...
    param = {
        "icao24": icao,
        "begin": int(datetime.utcnow().timestamp()) - 604800,
...
async def airport(travel_type, icao):
    ...
    if travel_type == "arrival" and icao != 1:
        ...
        response = await get(url=url, params=param)
        return response.json()
    if travel_type == "departure" and icao != 1:
